...
import {isTypeOfSafeNumber, stripNonNumber, valueAsNumber, zeroFill} from '@/utils/helper';
import {
  MAX_VALID_YEAR,
  MIN_VALID_YEAR,
...
export function isValidDate<G extends null | number | string | undefined>(value: G): value is G {
  // pt_br
  if (!isValidAny(value)) return false;
  if (!value) return true;
  if (typeof value !== 'string') return false;
...
export function isValidDateHour<G extends null | number | string | undefined>(value: G): value is G {
  // pt_br
  if (!isValidAny(value)) return false;
  if (!value) return true;
  if (typeof value !== 'string') return false;
