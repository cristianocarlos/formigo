import {arrayRemoveDuplicates, hasValue} from '@/utils/helper';
import {
  isSafeInteger,
  isValidCnpj,
  isValidCpf,
...
import type {TCheckAnyValue} from '@/lib/types/check';
import type {TCheckTableOrRadioTableRows, TRadioTableValues} from '@/lib/types/checkTableOrRadioTable';
import type {
  TFormigoAttribute,
...
type TCheckRequiredValidatorCallerProps = TFormigoValidatorBaseCallerProps & {
  checkValue: TCheckAnyValue | undefined;
};
...
type TRadioTableRowsCompleteValidatorCallerProps = TFormigoValidatorBaseCallerProps & {
...
export function comboBoxSelectedOnlyValidator(callerProps: TComboBoxSelectedOnlyValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    return attrValueDescription && !hasValue(attrValue)
      ? YiiLang.formigo('feedbackFormValidatorSelectedOnly')
      : undefined;
  };
}
...
export function cnpjValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidCnpj(attrValue) ? YiiLang.formigo('feedbackFormValidatorCnpjInvalid') : undefined;
  };
}
...
export function cpfValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidCpf(attrValue) ? YiiLang.formigo('feedbackFormValidatorCpfInvalid') : undefined;
  };
}
...
export function currencyValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidCurrency(attrValue)
      ? YiiLang.formigo('feedbackFormValidatorCurrencyInvalid')
      : undefined;
  };
}
...
export function dateValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidDate(attrValue) ? YiiLang.formigo('feedbackFormValidatorDateInvalid') : undefined;
  };
}
...
export function dateHourValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidDateHour(attrValue) ? resolveDateHourError(attrValue) : undefined;
  };
}
...
export function emailValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidEmail(attrValue) ? YiiLang.formigo('feedbackFormValidatorEmailInvalid') : undefined;
  };
}
...
export function floatValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidFloat(attrValue) ? YiiLang.formigo('feedbackFormValidatorFloatInvalid') : undefined;
  };
}
...
export function hourValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidHour(attrValue) ? YiiLang.formigo('feedbackFormValidatorHourInvalid') : undefined;
  };
}
...
export function integerValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isSafeInteger(attrValue) ? YiiLang.formigo('feedbackFormValidatorIntegerInvalid') : undefined;
  };
}
...
export function zipCodeValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue(attribute);
    return attrValue && !isValidZipCode(attrValue) ? YiiLang.formigo('feedbackFormValidatorZipCodeInvalid') : undefined;
  };
}
...
export function maxSizeValidator(callerProps: TMaxSizeValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    return size > maxSize ? YiiLang.formigo('feedbackFormValidatorMaxSizeSelectionError') + ': ' + maxSize : undefined;
  };
}
...
export function minSizeValidator(callerProps: TMinSizeValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    const attrValue = getAttrValue<Array<unknown> | NonNullable<unknown>>(attribute);
    const size = attrValue ? (Array.isArray(attrValue) ? attrValue : Object.keys(attrValue)).length : 0;
    return size < minSize ? YiiLang.formigo('feedbackFormValidatorMinSizeSelectionError') + ': ' + minSize : undefined;
  };
}
...
export function checkRequiredValidator(callerProps: TCheckRequiredValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    const attrValue = getAttrValue(attribute);
    return attrValue !== checkValue ? YiiLang.formigo('feedbackFormValidatorRequired') : undefined;
  };
}
...
export function descriptionRequiredValidator(callerProps: TDescriptionRequiredValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    return !hasValue(attrValue) ? YiiLang.formigo('feedbackFormValidatorRequired') : undefined;
  };
}
...
export function minSizeOneRequiredValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    ...
    return size === 0 ? YiiLang.formigo('feedbackFormValidatorRequired') : undefined;
  };
}
...
export function radioTableColsUniqueValidator(callerProps: TFormigoValidatorBaseCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute} = callerProps;
    const attrValue = getAttrValue<TRadioTableValues>(attribute);
    if (attrValue) {
      const attrValueRowsCount = Object.keys(attrValue).length;
      const attrValueColsCount = arrayRemoveDuplicates(Object.values(attrValue)).length;
      if (attrValueRowsCount > attrValueColsCount) {
        return YiiLang.formigo('feedbackFormValidatorRadioTableColsUniqueError');
      }
    }
    return;
  };
...
export function radioTableRowsCompleteValidator(callerProps: TRadioTableRowsCompleteValidatorCallerProps) {
  return (getAttrValue: TFormigoValidatorGetAttrValue) => {
    const {attribute, rows} = callerProps;
    const attrValue = getAttrValue<TRadioTableValues>(attribute);
    if (attrValue) {
      const attrValueRowsCount = Object.keys(attrValue).length;
      const rowsCount = rows.length;
      if (rowsCount > attrValueRowsCount) {
        return YiiLang.formigo('feedbackFormValidatorRadioTableRowsCompleteError');
      }
    }
    return;
  };
