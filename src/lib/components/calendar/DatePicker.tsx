...
export default function DatePicker(props: TProps) {
  ...
  return (
    <div
      className={`formigo--z-input-picker absolute left-0 origin-top-left -translate-y-1 scale-95 overflow-hidden rounded bg-white opacity-0 shadow-2xl transition-all duration-200 ease-in-out ${isOpen ? 'translate-y-0 scale-100 opacity-100' : ''} ${pickerPosition === 'right' ? 'right-0' : ''}`}
    >
      {isOpen ? (
        <Calendar
          className={className}
