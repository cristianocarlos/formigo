...
import {cpfValidator} from '@/lib/utils/validators';
import {
...
export default function CpfInput(props: TCpfInputProps) {
  ...
  useDidMountEffect(() => {
    inputPrepare(initValue, MASKS.cpf);
  });
  ...
  useValidators(
    ...
    (validators || []).concat([cpfValidator({attribute})]),
  );
  ...
  const resolvedInputValue = resolveInputValue(attrValue, undefined, 'CpfInput', attribute);
