...
import {integerValidator} from '@/lib/utils/validators';
import {
...
import type {TIntegerInputProps} from '@/lib/types/input';

export default function IntegerInput(props: TIntegerInputProps) {
  const {
    allowNegative,
    attribute,
    autoComplete = 'off',
  ...
  const {handleMaskInputBeforeInput, handleMaskInputBlur, handleMaskInputPaste} = useMaskedInputHandlers({
    ...
    disabled,
    handleInputBlur,
    mask: allowNegative ? MASKS.negativeInteger : MASKS.integer,
    maxLength,
    readOnly,
  });
  ...
  useDidMountEffect(() => {
    inputPrepare(initValue, MASKS.integer);
  });
  ...
  useValidators(
    ...
    (validators || []).concat([integerValidator({attribute})]),
  );
  ...
  const resolvedInputValue = resolveInputValue(attrValue, undefined, 'IntegerInput', attribute);
